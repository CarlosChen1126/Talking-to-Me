...
parser.add_argument(
	...
	help='Audio Features Save Root. (default: ../hubert_features)'
)
parser.add_argument(
...
output_root = os.path.abspath(args.output_feat_root)	### output root
...
if __name__ == "__main__":
	if not os.path.exists(os.path.join(output_root, "train")):
		os.makedirs(os.path.join(output_root, "train"), exist_ok=True)
	if not os.path.exists(os.path.join(output_root, "test")):
	...
	model = S3PRLUpstream("hubert_large_ll60k", refresh=False).to(device)
	model.eval()
	print(model)
		...
		if os.path.exists(os.path.join(data_root, "train", "seg", f'{name[:-4]}_seg.csv')):
			with open(os.path.join(data_root, "train", "seg", f'{name[:-4]}_seg.csv')) as dur_file:
				...
				for info in range(len(infos)-1):
					person_id, start_id, end_id, ttm = infos[info+1].split(',')
					...
					with torch.no_grad():
						...
						# 22nd layer
						hs = all_hs[21]
						hs_len = all_hs_len[21]
						print(hs.shape, hs_len.shape)
						assert hs_len.dim() == 1

						try:
							# torch.save(hs, f'./hubert_features/{name[:-4]}_{start_id}_{end_id}_{ttm}')
							# print(f"{name[:-4]}_{start_id}_{end_id}_{ttm[:-1]}"+".pt")
							torch.save(hs, os.path.join(output_root, "train", f"{name[:-4]}_{start_id}_{end_id}_{ttm[:-1]}.pt"))
						except:
							print(f'can not save {name[:-4]}_{start_id}_{end_id}_{ttm[:-1]}.pt')

		elif os.path.exists(os.path.join(data_root, "test", "seg", f'{name[:-4]}_seg.csv')):
			with open(os.path.join(data_root, "test", "seg", f'{name[:-4]}_seg.csv')) as dur_file:
				infos = dur_file.readlines()
				for info in range(len(infos)-1):
					...
					with torch.no_grad():
						...
						try:
							# torch.save(hs, f'./hubert_features/{name[:-4]}_{start_id}_{end_id}_{ttm}')
							# print(f"{name[:-4]}_{start_id}_{end_id}_{ttm[:-1]}"+".pt")
							torch.save(hs, os.path.join(output_root, "test", f"{name[:-4]}_{start_id}_{end_id[:-1]}.pt"))
						except:
							print(f'can not save {name[:-4]}_{start_id}_{end_id[:-1]}.pt')